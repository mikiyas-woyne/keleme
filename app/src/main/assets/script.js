...
function createGame(soundEffects, currentLevel, onGameOver, onVictory) {
    ...
    function onContinueClick() {
        if (lives > 0) {
            ...
            // Safe Respawn starting from the ground with the same score preserved!
            player.x = canvas.width / 2;
            player.y = canvas.height * 0.72;
            player.vy = 0;
            player.color = COLORS[Math.floor(Math.random() * COLORS.length)];
            cameraY = 0;
            highestYGenerated = canvas.height * 0.4;
            
            // Rebuild initial layout from bottom
            obstacles.length = 0;
            ...
            isInvulnerable = true;
            invulnerableTimer = 120; // ~2 seconds of flashing invulnerability
